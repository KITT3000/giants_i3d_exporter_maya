...
class CollisionMaskFlags:
    def __init__(self, xmlFilepath):
        ...
        self.presetsByName = {}
        self.presetsByMasks = {}
        self.defaultColFilterGroup = parseInt("0xFF")
        self.defaultColFilterMask = parseInt("0xFFFF_FBFF")

        self.conversionRules = {}

        self.loadCollisionMaskFlagsFromXML(xmlFilepath)

        conversionRulesXMLFilepath = path.normpath(path.join(path.dirname(__file__), "collisionMaskConversionRules.xml"))
    ...
    def loadCollisionMaskFlagsFromXML(self, xmlFilepath):
        ...
        try:
            ...
            defaultGroupElem = root.find('defaultGroup')
            if defaultGroupElem != None:
                defaultGroup = parseInt(defaultGroupElem.text)
                if defaultGroup != None:
                    self.defaultColFilterGroup = defaultGroup
                else:
                    print("Warning: CollisionMaskFlags: unable to parse 'defaultGroup' value '{}' in '{}'".format(defaultGroupElem.text, xmlFilepath))
            
            defaultMaskElem = root.find('defaultMask')
            if defaultMaskElem != None:
                defaultMask = parseInt(defaultMaskElem.text)
                if defaultMask != None:
                    self.defaultColFilterMask = defaultMask
                else:
                    print("Warning: CollisionMaskFlags: unable to parse 'defaultMask' value '{}' in '{}'".format(defaultMaskElem.text, xmlFilepath))
